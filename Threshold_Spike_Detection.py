...
if detect_spikes == 'True':
    ...
    if detect_threshold_spikes == 'True':
        ...
        # if seizure detection chosen, iterate through seizure file and find spikes that occur during seizures
        # label the spikes as seizures if true
        if detect_seizures == 'True':
            seizures_df = pd.read_csv(f'{output_dir}/Event_Detection_Data/Seizures/{current_channel}_Seizures.csv')
            spikes_df = pd.read_csv(f'{output_dir}/Event_Detection_Data/Spikes/Threshold_Spikes/{current_channel}/{current_channel}_threshold_spikes.csv')
            ...
            spikes_df.to_csv(f'{output_dir}/Event_Detection_Data/Spikes/Threshold_Spikes/{current_channel}/{current_channel}_threshold_spikes_with_seizures.csv', index=False)
