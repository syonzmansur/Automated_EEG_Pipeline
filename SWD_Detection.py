...
if detect_swds == 'True':
    os.makedirs(f'{output_dir}/Event_Detection_Data/SWDs', exist_ok=True)
    print("SWD Detection has been set to occur")
    sf = raw.info['sfreq']
    data = raw.get_data(units = "mV")
    # Detection of spike and wave discharges using parameters from Jeanne Paz in Das et al., 2023
    ...
    # if swds detected, append to csv file
    if swd is not None:
        swd_events = swd.summary()
        swd_events.to_csv(f'{output_dir}/Event_Detection_Data/SWDs/{current_channel}_SWD_events.csv')
    # if no swds, append no swds found
    else:
        data = {'Message': ['No swds found']}
        df = pd.DataFrame(data)
        df.to_csv(f'{output_dir}/Event_Detection_Data/SWDs/{current_channel}_SWD_events.csv')
